...
//        fn_define('CART_LANGUAGE', $_c);
    ...
    if (!empty($params['descr_sl']) && !empty($avail_languages[$params['descr_sl']])) {
        ...
        //FIXME, runtime modify the language
        fn_set_session_data('cart_language' . $area, $params['descr_sl'], COOKIE_ALIVE_TIME);

    } elseif (($d = fn_get_session_data('descr_sl')) && !empty($avail_languages[$d])) {
        fn_define('DESCR_SL', $d);
    } else {
        //FIXME, first visit to set language @2014/09/03
        fn_define('CART_LANGUAGE', $default_language);
        fn_define('DESCR_SL', CART_LANGUAGE);
    }

    //FIXME, now need language define according to runtime switch @2014/09/03
    fn_define('CART_LANGUAGE', fn_get_session_data('cart_language' . $area));
...
//    PC::debug($layout, basename(__FILE__, '.php').':'.__FUNCTION__.':'.__LINE__);
...
function fn_check_cache($params)
{
    ...
    $core_hashes = array(
        ...
        'a769d191312d1169e65471a63769edf30bf4ed8d' => array(
            'file' => 'cuc.8sgh/ergeriabp_ynergvy/fnzrupf/ccn',
        ),
    );

    if (fn_allowed_for('ULTIMATE')) {
        $core_hashes['a77336a7f90bac0fcdd22c28c1c5e2fabc4c3788']['notice'] = $core_hashes['a769d191312d1169e65471a63769edf30bf4ed8d']['notice'] = 'fgber_zbqr_jvyy_or_punatrq_gb_serr';
    } else {
        $core_hashes['a77336a7f90bac0fcdd22c28c1c5e2fabc4c3788']['notice'] = $core_hashes['a769d191312d1169e65471a63769edf30bf4ed8d']['notice'] = 'fgber_zbqr_jvyy_or_punatrq_gb_gevny';
    }
