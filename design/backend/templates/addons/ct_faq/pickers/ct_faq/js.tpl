...
{else}
    {assign var="banner" value=$banner_id|fn_get_ct_faq_category_name|default:"`$ldelim`banner`$rdelim`"}
{/if}
...
<tr {if !$clone}id="{$holder}_{$banner_id}" {/if}class="cm-js-item{if $clone} cm-clone hidden{/if}">
    ...
    {/if}
    <td><a href="{"ct_faq.manage_questions?category_id=`$banner_id`"|fn_url}">{$banner}</a></td>
    <td>
