...
{if $view_mode != "list"}
    ...
    {include file="buttons/button.tpl" but_id="opener_picker_`$data_id`" but_href="ct_faq.picker?display=`$display`&amp;picker_for=`$picker_for`&amp;extra=`$extra_var`&amp;checkbox_name=`$checkbox_name`&amp;aoc=`$aoc`&amp;data_id=`$data_id`"|fn_url but_text=$but_text|default:__("add_ct_faq") but_role="add" but_target_id="content_`$data_id`" but_meta="cm-dialog-opener btn pull-right" but_icon="icon-plus"}
...
{if $view_mode != "button"}
	...
	{include file="addons/ct_faq/pickers/ct_faq/js.tpl" banner_id="`$ldelim`banner_id`$rdelim`" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button position_field=$positions position="0"}
	{if $item_ids}
	{foreach name="items" from=$item_ids item="p_id"}
		{include file="addons/ct_faq/pickers/ct_faq/js.tpl" banner_id=$p_id holder=$data_id input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration}
	{/foreach}
	{/if}
	</tbody>
